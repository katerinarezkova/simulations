...
k = 3 # 1, 2, 3, 4
nu = Constant(5 * 10**(-k))                          # set (0,0) value (later used at u = (0,0) on boundaries)
...
def a(u, v_conv, phi):                                                             # u = solve, v_conv = fix, phi = test function
    a = (dot(dot(grad(u), v_conv), phi) + inner(nu*grad(u), grad(phi))) * dx                           
    return a         
...
F = a(v, v, v_) - b(p_, v) - b(p, v_) - inner(f, v_) * dx
...
# output files
out_v = VTKFile(f"results_CDN/v_sqr_steady_CDN_{k}.pvd")              # result of velocity
out_p = VTKFile(f"results_CDN/p_sqr_steady_CDN_{k}.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    ...
    print(f"- saved to results_CDN/v_sqr_steady_CDN_{k}.pvd")
    print(f"- saved to results_CDN/p_sqr_steady_CDN_{k}.pvd")
