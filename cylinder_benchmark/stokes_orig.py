...
# viscous term
def a(u, phi):                                      # u = solve, phi = test function
    D = sym(grad(u))                                # strain-rate tensor D (for physical properties)
    return (inner(2*nu*D, grad(phi))) * dx          # bilinear form
...
# output files
out_v = VTKFile(f"results/velocity_{Re}_orig_st.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_orig_st.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):      
    print(f"Finished computations for Re = {Re} with D(u), Stokes.") 
    print(f"- saved to results/velocity_{Re}_orig_st.pvd")
    print(f"- saved to results/pressure_{Re}_orig_st.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) with D(u), Stokes')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_orig_st.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re}_orig_st.")
    print(f"- saved to graph_lift_drag_{Re}_orig_st.pdf")
