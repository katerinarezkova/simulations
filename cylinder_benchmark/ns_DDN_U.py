...
U = 3.75                                            # maximal velocity at inflow
...
v_r = as_vector((0,0))                              # reference velocity (to be determined from Stokes flow, now = 0)
...
def a(u, v_conv, phi):                                                             # u = solve, v_conv = fix, phi = test function
    a = (dot(dot(grad(u), v_conv), phi) + inner(nu*grad(u), grad(phi))) * dx - 1/2 * neg(dot(u,n)) * dot((v - v_r), phi) * ds(8)                          
    return a         
...
# output files
out_v = VTKFile(f"results/velocity_{Re}_DDN.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_DDN.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    print(f"Finished computations for Re = {Re} with DDN, U = {U}.") 
    print(f"- saved to results/velocity_{Re}_DDN_U.pvd")
    print(f"- saved to results/pressure_{Re}_DDN_U.pvd.")

# plot drag and lift coefficients
if COMM_WORLD.rank == 0:
    import matplotlib.pyplot as plt
    
    drag_array = np.array(drag_list)
    lift_array = np.array(lift_list)
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) with DDN, U = {U}')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_DDN_U.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} with DDN, U = {U}.")
    print(f"- saved to graph_lift_drag__{Re}_DDN_U.pdf")
