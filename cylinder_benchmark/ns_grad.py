...
# output files
out_v = VTKFile(f"results/velocity_{Re}_grad.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_grad.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):  
    print(f"Finished computations for Re = {Re} with grad.") 
    print(f"- saved to results/velocity_{Re}_grad.pvd")
    print(f"- saved to results/pressure_{Re}_grad.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) with grad(u)')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_grad.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} with grad.")
    print(f"- saved to graph_lift_drag__{Re}_grad.pdf")
