...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    print(f"Finished computations for Re = {Re} with DDN.") 
    print(f"- saved to results/velocity_{Re}_DDN.pvd")
    print(f"- saved to results/pressure_{Re}_DDN.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) with DDN')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_DDN.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} with DDN.")
    print(f"- saved to graph_lift_drag__{Re}_DDN.pdf")
