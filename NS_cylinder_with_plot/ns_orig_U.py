...
def a(u, v_conv, phi):                                                             # u = solve, v_conv = fix, phi = test function
    D = sym(grad(u))
    a = (dot(dot(grad(u), v_conv), phi) + inner(2*nu*D, grad(phi))) * dx                          
    return a         
...
# output files
out_v = VTKFile(f"results/velocity_{Re}_orig_U.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_orig_U.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    print(f"Finished computations for Re = {Re} orig, change U.") 
    print(f"- saved to results/velocity_{Re}_orig_U.pvd")
    print(f"- saved to results/pressure_{Re}_orig_U.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) orig, U = {U}')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_orig_U.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} orig, U = {U}.")
    print(f"- saved to graph_lift_drag__{Re}_orig_U.pdf")
