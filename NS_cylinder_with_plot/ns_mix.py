...
# output files
out_v = VTKFile(f"results/velocity_{Re}_mix.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_mix.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    print(f"Finished computations for Re = {Re} mix.") 
    print(f"- saved to results/velocity_{Re}_mix.pvd")
    print(f"- saved to results/pressure_{Re}_mix.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) mix')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_mix.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} mix.")
    print(f"- saved to graph_lift_drag__{Re}_mix.pdf")
