...
def a(u, v_conv, phi):                                                             # u = solve, v_conv = fix, phi = test function
    D = sym(grad(u))
    a = (dot(dot(grad(u), v_conv), phi) + inner(2*nu*D, grad(phi))) * dx - ((neg(dot(u,n)) * dot((v - v_r), phi))/2) * ds(8)                          
    return a         
...
# output files
out_v = VTKFile(f"results/velocity_{Re}_mix_U.pvd")              # result of velocity
out_p = VTKFile(f"results/pressure_{Re}_mix_U.pvd")              # result of pressure
...
if COMM_WORLD.rank == 0:                         # for the main process (for MPI):
    print(f"Finished computations for Re = {Re} mix, U = {U}.") 
    print(f"- saved to results/velocity_{Re}_mix_U.pvd")
    print(f"- saved to results/pressure_{Re}_mix_U.pvd.")
...
if COMM_WORLD.rank == 0:
    ...
    plt.title(f'Flow around cylinder ($Re = {Re}$) mix, U = {U}')
    plt.xlabel('Time [s]')
    ...
    # save plt
    plt.savefig(f'graph_lift_drag_{Re}_mix_U.pdf', bbox_inches='tight')
    print(f"Finished plotting for Re = {Re} mix, U = {U}.")
    print(f"- saved to graph_lift_drag__{Re}_mix_U.pdf")
